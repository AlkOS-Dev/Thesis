...
The application calls \texttt{printf}, which internally buffers data and invokes the \texttt{write} system call. The successful appearance of text on the debug terminal confirms that the Global Descriptor Table (GDT) is correctly configured for user-mode execution and the syscall dispatcher correctly routes requests.
...
This sequence validates path resolution, file descriptor table management in the kernel, and the read/write logic of the filesystem driver.
...
The application begins by invoking the \texttt{create\_graphic\_session} system call. This request instructs the kernel's Window Manager to allocate a physical memory buffer acting as a backbuffer for the process. The kernel then maps this physical memory into the virtual address space of the process and returns a \texttt{GuiBufferInfo} structure.
...
Once the session is established, the application treats the video buffer as a raw array of `uint32\_t`. It implements a rendering loop that generates a moving pattern by calculating pixel colors based on coordinates and a time-variable offset.
...
The shell implements a Read-Eval-Print Loop (REPL). It reads input from `stdin`, which is connected to a kernel pipe fed by the keyboard driver. The `ProcessCommand` method process the input string and dispatches execution.
