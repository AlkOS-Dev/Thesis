...
\subsection{Implementation of the Standard Library}
...
\subsubsection{Core Identification}
...
\subsubsection{Block Device Abstraction}
...
\subsubsection{The Bootstrapping Problem and Initrd}
...
\subsubsection{Scheduler Internal Synchronization}
...
Finially to add sleep we need to add two functionalities to the code: putting the task into the sleep and waking up the task. First is pretty straightforward, similar to blocking we change the location of thread from scheduler queues to sleeping data structure and change the state. In the other the situation is slightly more complicated as we first need to find a proper place where tasks are woken up. In case of cooperative shceduling it simply may be inside function picking next task from the scheduler queues. In other cases we have to manage timing framework and wake up threads carefully to not introduce additional latencies. 

For ticking kernels sleep with granularity around the tick is pretty straightforward, waking up threads each time we have interrupt is sufficient. Very simple solutions exists for managing this type of sleeping tasks \cite{linux_timer_wheel}.

For other case where we want to schedule interrupts dynamically the situation is quite complicated. It requires proper sorting of the tasks (creation of timeline), that means some priority queues will be involved here. For nanosleep if the tasks want to sleep for really small value, for example smaller than time needed to interact with the timing module or timer reconfiguration time it might be worth to introduce busy waiting instead. Designing such framework timing constraits must be taken in account, like time spent on the timer configuration or if the time left is not smaller than the reconfiguration time. It might be really tricky and difficult as in some cases we will be fighting with time. Additionally each time we reprogram the timer we should wake up tasks that previous timer should woke up if the value is small enough.

\subsection{User Space}

Finally we have all components to introduce the possibility to run unprivileged tasks create by the typical user. Briefly to enable running user space programs we still need  couple of things:

\begin{itemize}
    \item syscalls -- communication with the kernel. Way to escalate the privilege
    \item user space libraries -- porting the libraries for the kernel
    \item loading the executable from fs -- the kernel must understand some executable format and properly load the code to the memory
    \item proper memory protection -- separate address spaces and privilege flags setup
    \item conversion kernel -> user space -- kernel space bootstrap code at the end should jump to user space and start executing users code
    \item user space compilation -- extending the compiler and enable compilation targeting our os
\end{itemize}
...
Since threads frequently migrate between different linked lists and data structures, allocating and freeing list nodes for every operation introduces significant overhead. To address this, the system employs intrusive data structures (Listing~\ref{lst:intrusiveNodes}). Instead of allocating a node that contains a pointer to the object, the node structure is embedded directly within the object itself. Operations on the lists are performed by simply modifying the fields of the object.
