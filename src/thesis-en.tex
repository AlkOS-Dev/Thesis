...
struct alignas(arch::kCacheLineSizeBytes) ClockRegistryEntry : data_structures::RegistryEntry {
  ...
  u64 ns_uncertainty_margin_per_sec;  
    // Uncertainty margin in femto seconds per second
...
\subsubsection{TSC \cite{IntelManual-TSC}\cite{osdev-tsc}}

The Time Stamp Counter (TSC) is the optimal clock source due to its high precision and core-locality. Accessing the TSC involves reading a CPU register, which requires only a few cycles, unlike external clocks that may require hundreds. However, the TSC has historical limitations. On older processors, the counter's frequency was tied to the core frequency. Consequently, frequency scaling (throttling) caused the time measurement to drift, rendering it unreliable. Modern Intel processors introduced the Invariant TSC, which ensures a constant frequency regardless of the core's power state. Another limitation is that on certain CPUs, the TSC frequency is not explicitly known and must be measured against a known reference. For this purpose, the system utilizes the HPET, which serves as the minimal hardware requirement for reliable calibration.

\subsubsection{HPET \cite{Intel-HPET}\cite{osdev-hpet}}
...
For x86-64, the selection of event clocks is based on the following priority order: LAPIC Timer > HPET > PIT. Currently, only the LAPIC Timer is supported.

\subsubsection{LAPIC Timer \cite{osdev-lapic-timer}\cite{IntelManual-LAPIC-Timer}\cite{IntelManual-APIC}}
...
Final part of the OS, which strongly depends and utilizes all other modules. Our implementation targets only single core for simplicity, but the design might be easly extended to multiple cores, which will be described at the end of this document.

\subsection{Process Structure}

Our definition of process looks like that:
...
As said in limitations\ref{subsec:limitations}, maximal amounts of processes exists and we use that property to speedup process lookup by building identity 1:1 hash map for processes and embedding reservable identifier inside of the PID. The other part of PID is simply atomic counter, which makes the PID unique for the entire lifetime of the system.

\subsubsection{Process Flags}

\textbf{KernelSpaceOnly} flag enables creation of kernel space only threads and processes that means there is no creation of address space or user space stack, leadining to smaller resource consumption and little spedups for example when we will be switching context from kernel thread to kernel thread there is no need to change the address space at all.

\textbf{PreserveFloats} enabling this flags makes all threads save and load float registers during the context switch. Additionaly user may want to disable this flag for particular thread.

\subsubsection{Process State}

It has a less significant meaning than Thread State (which will be described later), it is mostly used for proper resource management during waiting for the process. Ensures that the process is not cleaned up twice.

\subsubsection{Address Space}

The most important part of the process structure. It defines all informations about virtual memory, more precisly establishes translation between virtual and physical layer. Each process operates in own memory address space, that allows for much simpler memory management including fragmentation, perofrmance and security. Refer to Memory Managment\ref{sec:memory} for more information.

\subsubsection{File Descriptor Table}

This table contains all file descriptor opened by the threads running within this process. Please refer to File System \ref{section:fs}

\subsubsection{Pipes}

Buffored I/O of the process. Mostly for reading and writing with the user.

\subsubsection{Name}

Name of the process, most frequently name of the executable. May not be unique. Pid is unique.

\subsubsection{Threads}

Intrusive doubly linked list of threads running in this environment, used mostly for kill, exit or wait operations.

\subsubsection{Live Threads and Threads to Clean}

Counters for synchronization with \textbf{ProcessRipper}. Refer to Kernel Workers \ref{subsubsec:kworkers} for more details.
 
\subsubsection{Status}

Exit status of the process to be passed to waiting thread.

\subsubsection{Wait Queue}

\textbf{Wait Queue} used to store threads waiting for this process to finish. Each of them will be woken up when the process finished its execution.

\subsection{Kernel Address Space}

There is only one Kernel Address Space in the entire OS. Each kernel process is working inside this address space. Additionally each userspace process's address space had kernel addres space mapped for performance and ease of life reasons. User threads will be calling system calls quite frequently, changing address space on each syscall would infer huge performance cost and would additionally complicate syscall logic.

\subsection{Thread Structure}

Our definition of thread looks like that:
...
Similarly to PID \ref{subsubsec:pid}, we limits the amout of available threads and we use that property to create unique TID allowing for fast lookups.

\subsubsection{Owner's PID}

PID of the process we are operating in.

\subsubsection{Thread Flags}

Collection of bitfields altering how the scheduler treats the thread.

\textbf{SchedulingPolicy policy} defines in which policy the thread is currently operating in. Refer to Policies \ref{subsec:policies} for more details.

\textbf{u8 priority} defines how important is our thread inside the policy we operate in. Internal for the kernel.

\textbf{UserPriority user\_priority} may be specified by the User Space to inform the scheduler that some threads are more important than others.

\textbf{bool preserve\_floats} specifies whether on context switches floting point state is preserved.

\textbf{bool detached} specifies whether scheduler should automatically cleanup the thread without waiting for joins.

\subsubsection{Thread State}

Describes what is currently going on with the thread. Allows us to nicely track each thread individually and verfiy the integrity of the system as only couple of state conversions are possible.

\subsubsection{Stacks}

For each thread we need two distinct stacks, one for the user and other for kernel. We need a separate stack for kernel for security and stability issues. There is also not efficient to have single stack for kernel as we also want to make context switches inside the kernel code. For example during thread joining or other synchronization.

\subsubsection{Statistics}

Allow us trace each threads execution specifics and adjust the scheduling module dynamically or simply debug/test the scheduler.

\subsubsection{Wait Queue}

Allow us to block other threads and wait for that thread to finish execution (Thread joining).

\subsection{Kernel Workers}
\label{subsubsec:kworkers}

The OS also before converting to the scheduling module create three kernel workers, which are crucial for the system:

\textbf{Trace Dumper} - this worker is responsible to dump the kernel traces to the terminal or files to enable debugability and stability. This cannot be done by kernel code itself as writing to devices or files take much too long for system calls or interrupts. The buffers are dumped periodically each 20ms

\textbf{Thread Ripper} - this worker is responsible for deallocation of thread resources including descriptors or stacks.

\textbf{Process Ripper} - this worker is responsible for deallocation of process resources including address space.

\subsection{Intrusive Data Structures}

Because threads will be frequently changing linked lists or data structures they reside in, idea to each time allocate and free list nodes looks rather stupid so we used introduced schemue to always use intrusive data structures. That means insted of allocating a node and storing field with pointer to the object, the node itself resides inside the object. We allocate the object and simple modify object fields to operate on the lists.
...
We can see on Thread data structure how to use such nodes \ref{lst:threadStruct}. Such approach has multiple benefits for, which includes:
\begin{itemize}
  \item No allocation or deallocation during instert or pop operations
  \item Better memory acces, in traditional model we have to acces object by pointer, here we directly acccess the object
  \item Verify if object belongs to some list or not in \textbf{O(1)}, having only the object itself
  \item Removal of the object from the list done always in \textbf{O(1)}
  \item Removal of the object from the list without accessing the list itself
\end{itemize}

\subsection{Meta-Scheduler}

The base arichtecture of the scheduler is similar to used in most of the popular that is \textbf{meta-scheduler}. This approach is based on bulding all scheduler operations on abstractions called \textbf{Policies}. Policies are mostly responsible for picking next task from set of tasks that currently belongs to the policy and react on thread behaviour (punish or reward it with CPU time based on work characteristics). Wheras the scheduler is responsible for all other operations, which includes 
\begin{itemize}
  \item managing thread states,
  \item putting threads into sleep
  \item waking up threads
  \item idling
  \item setting up next time events by timing infrastructure
  \item blocking threads on wait queues
  \item releasing threads from wait queues
\end{itemize}
There exists hierarchy between the policies and threads from more important ones are picked before threads from less important ones.

\subsection{Timing Model}

We decided to take more modern approach and implement tickless kernel which instead of firing interripts at constant frequency (so called \textbf{Kernel Tick}) - \textbf{Ticked Kernel}, we calculate precisely when next timing event should occur.

\subsection{Policies}

The type of the policies we defined in our code are following:

\textbf{Round Robin Scheduling Policy} -- Very simply policy iterating through linked list from front to end. Threads are ordered by time of added to the policy

\textbf{Priority Queue Scheduling Policy} -- Very simple policy ordering threads thgrough kernel assigned priorities. The priorities are capped to 0-64 range to use \textbf{O(1)} priority queue called \textbf{Bitmap Priority Queue}. The idea is that we store table of linked lists, which represents individual priorities and additionaly we store information about present priority inside the integer and on each lookup we use bit opreation count leading/trailing zeros to find the minimal or maximal value in the queue.

\textbf{Multi-Level Feedback Queue (MLFQ) Scheduling Policy} -- TODO ADAM

\noindent We use this type of priorities to define the hierarych which is given by that enum:
...
Each policy is used for different group of tasks:

\textbf{Uber Tasks} -- tasks that must be done as fast as possible, most probably rescue actions or some kernel space saving. For example saving state before shutdown or killing some processes when system is out of the memory. 

\textbf{Drivers} -- driver tasks that must be done as fast as possible to not block the devices and enable smooth operation of the system. For example audio drivers or net interfaces.

\textbf{Urgent Tasks} -- tasks that are less important that the drivers itself but more important than user tasks. Possibly privliged userspace tasks consuming the driver informations.

\textbf{Normal Tasks} -- most popular group of tasks there are all usual userspace programs inclduing graphical interfaces, data processing etc. Also basic kernel workers are here, including workers listed in kworkers section \label{subsubsec:kworkers}.

\textbf{Background Tasks} -- group of tasks that are not relevant at all, but may be done when there is nothing to do. It may include update checks or some non urgent cleanup tasks.

\subsubsection{Policies Abstraction}
\label{subsec:policies}

The abstraction of policies is following:
...
As we can see the most imprtant operation is \textbf{pick\_next\_task}, obvious \textbf{add_task} and \textbf{remove_task} and finally some helpers for the scheduler or statistics gathering.

\subsection{Sleeping}

To provide high precision sleeping we had to work on oneshot precise timing events. That means each time we receive and timing event we must deduce the time for the next event based on schedulers state, that include current thread preemption time or wakup times of sleeping threads. That means each time we receive timer interrupt we must look through sleeping threads. To ensure high performance and low stable complexity of priority queue we decided to use Intrusive Red-Black Tree (Linked list as node and storing min/max on insert/delete) as base data structure for \textbf{Sleeping Queue}. The operation we cared the most are \textbf{Insert}, \textbf{ExtractMin} and \textbf{Remove}, also the execution times must be predictable and stable. The requirement for quick removal excludes Binary priority queues and requirement for stability excludues all heaps operating on amortized complexities. The RB Tree was good enough and was also needed in other places so the choice was obvious. 
