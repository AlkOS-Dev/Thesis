...
Additionally we should consider how we will be testing our code and how to make it efficient. In that place emulation comes in place. We have two major paths we can follow. We can either have some testing machine and each time create the image and boot it up on the machine. It is even nevessary to verify the correct behaviour of the kernel on real hardware, but it may be time consuming for develeopment type of runs and can be a sort of bottleneck. To address that issue we have multiple emulation solutions available on the market including qemu, bochs or virtual box. This tools allow us to specify the target machine architecture and run it locally inside the host system in emulated environemnt. Such setup may speedup quick reruns checks or debugging or simple space exploration inside the virtual environment. The ideal solution is to combine both of these to maximize both development speed and quality.
...
\section{Kernel Space Allocation API}
TODO: KRYCZKA
