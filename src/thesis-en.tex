...
The primary objective of this thesis, as initially defined, was to implement a functional scheduler capable of executing tasks within isolated address spaces. Specifically, the project aimed to achieve the following minimal technical goals:
...
During the development we used two type of tests, which we found very helpfull  and wsa necessary to deliver the quality inside of a such big project:

\begin{itemize}
    \item Unit Testing - we implemented multiple unit tests inside the kernel both to verify correctnes of libc/libcpp and internal kernel components
    \item Regression Testing - parsing kernel log for errors inside CI ensuring no warning or fatal information is displayed
\end{itemize}

Finally we tested the project manually using multiple example programs we provided \ref{sec::example_programs}.


\section{Conclusion}

Summing up we finished the project fulfilling our initial requirements and even extended the scope by multiple features. Subjectively speaking we are proud of our archievement including the final shape of the project and work we have done. We overdone our expectetions and finished our main goal that is we deeply understood multiple concepts involved during os development. Objectively speaking we delivered more than expected, reading on the way hundred of pages of Intel Software Deleoper Manual, reading over a hunder documents on osdev, reading hundreds of osdev forum posts and discussions, went through thousands of lines of external os projects like linux or minix, working with multiple specifications or hardware documentations. Looking into numbers we finished at this point with over 80000 lines of code and total of around 700 files in the codebase. Critically looking we didnt do everything perfectly, we stumlbed upon multiple times on the way. We had to refactor multiple thousands of code lines because of teh debt or something we initially didnt understand well. Build system was rewriten fully at least 2 times and it sill needs some refactorings. Also at the begining without understandment how something function we implemented some module and after couple of weeks we rewrote the whole thing as we understood the problem better. Also we made multiple bad design decision, mostly with include patterns and project structure, finally everything was fixed, but initially we didnt predicted that accepting some scheme some edge case would create multiple circular dependencies. I think we did a good job with work separation as finally everyone did similar part of the project and at any point everyone was busy, but to be honest complexity of the project simplified the issue for us as there was more problems to solve than people we had. Another thing we could do better during the process is better communication during designing  things. Finally we probably spend too much time working with libc and libcpp implementation, but finally it also was very educational and horizon-expanding. 
