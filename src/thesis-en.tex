...
Before any code can run, the hardware must achieve a known state. 

TODO - Explain \\
1. The difficulty of unifying that, since it's strictly hardware dependent \\
2. For this reason we expect a given architecture to just load the final kernel into memory \\
   with a given well defined state
3. what sort of state do we expect the bootloader to load our kernel into \\

\subsection{x86-64 Bootloading}

Following a power up, or resetting the reset pin, each processor on the bus performs a hardware reset.
Since most modern cpu's are multicore - they first execute an internal negotation over which one runs code, and which fall asleep. (Mulit Processor (MP) Initialization protocol)
The processor selected through this process, later called Bootstrap Processor starts immediately. And starts executing the firmware code. Every other core enters a halt state and waits for a special startup interrupt to wake it up.
When the signal drops,  the CPU registers are hardwired to specific values.
The CPU may perform a Built in self test on itself. 

It then immediately fetches and executes the instruction located at accesses the physical address 0xFFFFFFF0. This is mapped to firmware, either modern UEFI or legacy BIOS.


\subsubsection{Firmware}
- {BIOS}
If the machine uses BIOS (Basic Input / Output System)
The cpu starts in 16bit real mode, it can only access first 1MB (2^20bytes) of RAM. 
It uses segmented addressing. 
It runs a power on self test (POST) then
iterates over boot devices (CD, HDD, USB). It reads the first sector of them
512 bytes
it checks the last two bytes of this sector. they need to be 0x55 and 0xAA.
if it exists, bios copies that entire segment into physical ram and makes the cpu jump there

- {UEFI}
If the machine is using Unified Extensible Firmware Interface, the process is more sophisticated.
Unlike BIOS which only read sectors, UEFI has filesystem awareness (can read fat32), and looks for a specific file on
an EFI file system partition. It then loads the executable into ram and executes it.
Notably it doesnt stay in 16 bit mode, and switches to 32 bit protected mode.

\subsubsection{Multiboot2}

Ultimately, no matter if BIOS or UEFI, it ends up loading another piece in the chain, a bootloader,
This program will in our case be grub, but could be any multiboot2 compliant bootloader.
It normalizes the environment, so the kernel doesn't care if it was loaded with BIOS or EFI. 
It also utilizes code exposed by UEFI and BIOS to scan for important hardware related data, such as the memory map, or information about the framebuffer. It also normalizes it. Getting this information after we quit uefi/bios services is hard
It loads the kernel and all asociated modules (binaries) from the device to the ram, and gives control to the kernel.

The cpu is loaded in 32-bit protection mode. Paging disabled. Interrupts disabled. The EAX register confirms that that kernel was loaded with multiboot compliant bootloader, while the EBX 

This state is inappropiate for the kernel. First - we're only in 32 bit mode and not 64. Second. Paging is disabled, so we dont have access to the higher half of memory, where the kernel should reside.
Third. There is no stack, so C won't work. Most cpu features are still disabled (like floating point registers or AVX)
...
The scheduling module is the final core component of the operating system, relying heavily on and utilizing all previously described modules. Although the current implementation targets a single-core architecture for simplicity, the design is extensible to multi-core systems, as discussed later in this document.
...
As mentioned in the limitations section \ref{subsec:limitations}, the system imposes a hard limit on the maximum number of existing processes. This constraint allows for optimized process lookup by establishing a 1:1 identity hash map. The \textbf{PID} (Process Identifier) is composed of a reservable identifier and an atomic counter, ensuring that each PID remains unique throughout the entire lifetime of the system.
...
\textbf{PreserveFloats} -- informs that all threads within the process save and restore floating-point registers during context switches by default. Disabling this flag allows the system to omit these potentially expensive operations for threads that do not utilize floating-point arithmetic.
...
This field points to an first element of doubly linked list of threads running in this environment. It is used primarily for operations such as termination (kill/exit).
...
Describes the current status of the thread. This state tracking allows the kernel to monitor each thread individually and verify system integrity, as only specific state transitions are permitted.

\subsubsection{Stacks}

Two distinct stacks are required for each thread: one for user space and one for kernel space. A separate kernel stack is essential for security and stability. Furthermore, sharing a kernel single stack is inefficient because context switches may occur within kernel code, such as during thread joining or other synchronization events.
...
\textbf{Trace Dumper} -- responsible for dumping kernel traces to the terminal or files, ensuring debuggability and stability. This task cannot be performed directly by the kernel code during critical execution paths, as writing to devices or files is too slow for system calls or interrupt handlers. The buffers are dumped periodically every 20ms.
...
As shown, the primary operation is \textbf{pick_next_task}, alongside standard \textbf{add_task} and \textbf{remove_task} operations. Additionally, helper functions are provided for the scheduler and for statistics gathering.

\subsection{Sleeping}

To provide high-precision sleeping, the system relies on one-shot, precise timing events. Upon receiving a timer interrupt, the scheduler determines the timestamp for the next event based on the system state, which includes the current thread preemption time and the wake-up times of sleeping threads. Consequently, every timer interrupt necessitates a traversal of the sleeping threads. To ensure high performance and stable time complexity for the priority queue, an Intrusive Red-Black Tree was selected as the underlying data structure for the \textbf{Sleeping Queue}. The critical operations are \textbf{Insert}, \textbf{ExtractMin}, and \textbf{Remove}, all of which must have predictable execution times. The requirement for efficient arbitrary removal excluded binary heaps, while the need for stability excluded heaps with amortized complexity bounds. The Red-Black Tree satisfied all requirements and was already implemented for other system components, making it the optimal choice.
