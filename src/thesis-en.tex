...
Finally when we have other components done we can proceed to the last major component which is needed - scheduler. It will be responsible for managing, distributing cpu time between runing tasks and blocking tasks if there is a need to wait on some specific event.

\subsubsection{Process}

First element we will have to implement to have some runnable tasks is structure and management of processes. This object will represent execution space of threads working inside of it. That means it must contain all resources shared between the threads. That will surely include:

\begin{itemize}
    \item Paging tables -- archtiecture defined structure describing mapping between virtual and physical addresses
    \item Mapping metadata -- strutures allowing to find and allocate virtual addresses
    \item File Descriptor Table -- descriptors allowing to access the files
\end{itemize}

All other resources added to the process are some cosmetics allowuing to implement various features like IPC, pipes or synchronization. Those 3 are bare minimum essential for the process concept. The straightforward solution is to implement separate struct for Thread and Prcoess objects, but there exists solution merging both of them to single task struct \ref{subsec:linux-task}. At this moment it is good to prepare some data strcutures allowing for fast quierying for processes for operations such killing, jojning or IPC. It is important to note that creation and destruction of processes as it needs to build the page tables map the kernel address space takes, and allocate the memory for all relevant structures takes a lot of time (similarly destruction). Some work might be defferred to external kernel tasks to not spend so much time in kernel space and return earlier to the thread creating or destruction the process.

The minimal stuct may look like this:
...
Obviously in reality this struct would contain much more data used for management like synchronization, statistics, configuration flags or state enum for lifecycle management mentioned before.

\subsubsection{Thread}

Second big block of scheduling is thread. It represents the unit of execution, some actual code which will actually use the CPU time and utilize the resources from its process (environment it operates inside). What we need to restore, startup and switch execution? Firstly the thread's stack and after that all state associated with that is all registers, address of next instruction and all archtiecture defined registers (flags, thread local storage etc). All of that compresses simply to one filed - thread's stack. This solution simplyfies life a lot and also performs well - as stack's memory is usually cached (it is used all the time) and we don't have to allocate additional memory for bookkeeping resources. That means we will Unify all operations to use stack as storage for threads state, we will dump the registers on top of it when saving the context or artificially fill up the stack with bootup values.

\noindent We can start with such simple stuct:
...
Another important field and enum, which we have to define is Thread's state. Based on that field we will be recognizing the action we have to take perform some operations. For example if we will have to kill some thread, based on the state we will do different things. For waiting thread we will have to remove it from scheduler for blocked threads we will have to remove the from wait queue its waiting on. 


\noindent We can now update our structure:
...
Other important things to consider now is that what happens if we will want to save the state on context switch on users stack? Doing is ultimately unsecure, malicious program can replace stack pointer with some address on imporatnt kernel structures and affect the behaviour or after going back to program we can leave some secret data like cryptographic keys or kernels state unintentionally. What can lead to further escalation as a consequence. Additionally we want to allow kernel code to preempt or block (that is make a context switch inside of kernel code), to do so we need to preserve the kernel state that is save the registers similarly as described before together with kernel stack. The consequence is we must have separate stack for the kernel per thread, having single stack per core is practically impossbile.

\noindent Final minimal stucture will look like this:
...
At this point we already know what our context is, but next important question is how to create the context. To initialize the context we have to create function, which will fill the context frame on the stack accoridngly to some scheme we will have to define. Such scheme will have to create a layout for registers and cpu flags discussed before. To create a context we will simply fill the values for all the registers (zeros for general purpose registers or function arguments, depends on used ABI), address of function we want to execute as a next address and flags as some architecture defined defaults.

\subsubsection{Context Conversion}

Before talking about switching a context we have to first create a first context we will have to switch afterwards. That means we have to create our first process and prepare all structures described before together with context initialization \ref{subsec:context-init}. We have multiple solutions here. For example we can create some first user space program which will bootup other programs or kernel space process perofrming further initialization.

\subsubsection{Context Switch}

When we have first couple of tasks available we may talk about switchin the context. Assuming that whole state resides on the stack the operation of context switch is pretty straightforward we will have to perform this operations in sequnce to change the context:

\begin{itemize}
    \item Push current thread context to its stack
    \item Do all additional actions needed
    \item Change the stack to next threads stack
    \item Restore the state from the stack
\end{itemize}

Based on amd64 assembly it might look like this:
