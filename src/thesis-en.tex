...
\subsubsection{Adam Ogieniewski}
...
\begin{enumerate}
    ...
    \item \textbf{External (Decoupled):} Common in userspace libraries (e.g., \texttt{std::shared\_ptr}), where the control block is allocated separately and the smart pointer stores two pointers.
...
Most kernels employ the \textbf{intrusive reference counting model}. Although external models offer greater flexibility by avoiding modifications to object layouts, their associated costs are not justifiable within the kernel context. The intrusive model ensures that the reference counter is colocated with the object itself, which guarantees a single allocation per object. Furthermore, with an intrusive design, any subsystem holding a raw pointer can acquire ownership by incrementing the embedded counter, allowing objects to move freely between low-level components and higher-level abstractions without compromising correctness.

The usage of reference counting offers multiple benefits critical for systems programming:
\begin{itemize}
...
\begin{lstlisting}[caption={Polymorphic Page Metadata Structure}, label={lst:page_meta}]
struct PageMeta {
    union {
...
\begin{lstlisting}[caption={Bootstrap Linear Allocation Logic}, label={lst:bitmap_alloc}]
...
\begin{lstlisting}[caption={Buddy System State Management}, label={lst:buddy_lists}]
class BuddyPmm {
public:
...
\begin{lstlisting}[caption={Calculating Buddy PFN}, label={lst:buddy_xor}]
...
\begin{lstlisting}[caption={Coalescing Logic}, label={lst:buddy_merge}]
...
\begin{lstlisting}[caption={Compile-Time Slab Efficiency Calculation}, label={lst:slab_efficiency}]
...
\begin{lstlisting}[caption={Aligned Allocation Strategy}, label={lst:aligned_alloc}]
...
\begin{lstlisting}[caption={Physical to Virtual Address Translation helpers}, label={lst:phys_to_virt}]
template <typename T>
...
\begin{lstlisting}[caption={Generic Page Flags Abstraction}, label={lst:page_flags}]
struct PageFlags {
    bool Present : 1;
    bool Writable : 1;
...
Unlike anonymous memory, this area ignores the "Not Present" fault mechanic for allocation; it simply maps the pre-existing physical range.
...
\begin{lstlisting}[caption={Lazy Kernel Synchronization Logic}, label={lst:kernel_sync}]
...
The transition from the bootloader to the kernel's internal memory management is a critical phase. When the kernel main function receives control, the system is running on page tables created by \texttt{Loader64}. These tables contain a temporary identity mapping (0-10 GB) that is necessary for the loader but undesirable for the kernel, as it exposes physical memory to user-space (NULL pointer dereferences would be valid accesses to physical address 0).
...
\begin{lstlisting}[caption={Gap Search Logic}, label={lst:find_gap}]
...
\begin{itemize} 
    ...
    \item \textbf{FAT12/16/32 Support} -- The system supports the FAT12, FAT16, and FAT32 filesystem variants. Long file name (LFN) support is not implemented; filenames are therefore limited to the 8.3 format.
    \item \textbf{Ramdisk} -- The system is bootstrapped using an in-memory filesystem mounted as the initial root filesystem. Loading a root filesystem from persistent storage is not supported, and thus no permanent modifications are possible. The expected filesystem is hardcoded to FAT16.
    \item \textbf{ELF64} -- Userspace applications must be compiled as ELF64 binaries. Dynamic linking is not supported. In particular, binaries containing a \texttt{PT\_INTERP} segment \cite{elf-spec-dynamic} or relying on shared libraries cannot be executed.
\end{itemize}
...
\begin{itemize} 
    ...
    \item \textbf{Max Path Length} -- 1024 -- The absolute maximum length of a file path string accepted by the VFS.
    \item \textbf{Max Path Depth} -- 64 -- The maximum number of directory components (slashes) allowed in a single path.
    \item \textbf{Max Filename Length} -- 128 -- The maximum length permitted for a single file or directory name component.
    \item \textbf{Max Open Files per Process} -- 128 -- The hard limit on file descriptors allocated to a single process.
    \item \textbf{Max System Open Files} -- 1024 -- The capacity of the global open-file table, limiting the total number of files that may be open system-wide at any given time.
    \item \textbf{Max Active Files Cache} -- 512 -- The limit of the internal file objects, restricting how many unique files can be actively referenced by the system simultaneously.
    \item \textbf{Pipe Buffer Size} -- 4~KB -- The fixed circular buffer size used for standard I/O streams.
    \item \textbf{Max Heap Size} -- 16~GB -- The maximum virtual address range available for a userspace process's heap.
    \item \textbf{Max Syscall Arguments} -- 6 -- The maximum number of arguments that can be passed to a system call.
\end{itemize}
...
\begin{itemize} 
    ...
    \item \textbf{Filesystem Caching} -- Introduce caching to improve filesystem performance.
    \item \textbf{Filesystem Permissions} -- Implement file and directory permissions for security and access control.
