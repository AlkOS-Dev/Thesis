...
The DTB format encodes the device tree into a linear, pointerless data structure, as illustrated in Figure~\ref{fig:dtb_structure}. It consists of a fixed-size header followed by three variable-sized blocks:
\begin{enumerate}
...
To validate the functionality of the AlkOS kernel and its subsystems, a set of userspace applications was developed. these programs serve as integration tests, verifying the interactions between the system call interface, the C standard library, drivers, and the kernel core. Each program targets specific subsystems and features of the operating system.

\subsection{Hello World}

The \texttt{hello\_world} program serves as the verification of the user space environment. The implementation was extended to validate the VFS and the standard I/O library implementation.

\subsubsection{System Call Interface Verification}

The primary objective of this application is to validate the correct transition from user mode to kernel mode, to verify the proper operation of the system call interface, and to confirm successful initialization of the C runtime environment.

The application calls \texttt{printf}, which internally buffers data and invokes the \texttt{write} system call. The successful appearance of text on the debug terminal confirms that the Global Descriptor Table (GDT) is correctly configured for user-mode execution and the syscall dispatcher correctly routes requests (Listing~\ref{lst:hello_stdout}).
...
\subsection{Graphics Test Program}

The \texttt{gui\_test} application demonstrates the capability of the kernel to provide userspace processes with direct access to video hardware in a controlled manner. The resulting output is shown in Figure \ref{fig:gui_test}.
...
Once the session is established, the application treats the video buffer as a raw array of `uint32\_t`. It implements a rendering loop that generates a moving pattern by calculating pixel colors based on coordinates and a time-variable offset (Listing~\ref{lst:gui_render}).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{res/gui_test.png}
...
The shell provides a text-based command-line interface. This is achieved through the `GraphicsConsole` class, which renders text onto a graphical surface using the PSF2 font format. The console handles text rendering, cursor management, and screen clearing.

\subsubsection{Command Line Interpreter}

The shell implements a Read-Eval-Print Loop (REPL). It reads input from `stdin`, which is connected to a kernel pipe fed by the keyboard driver. The `ProcessCommand` method processes the input string and dispatches execution (Listing~\ref{lst:shell_process}).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{res/shell_welcome.png}
...
The shell includes a suite of internal commands designed to facilitate system inspection and file system navigation. These commands are handled directly within the \texttt{Shell::ProcessCommand} routine without spawning new processes.
...
\begin{itemize}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.9\textwidth]{res/shell_help.png}
    ...
    \item \textbf{echo <text>}: Prints the provided arguments to standard output. This command is primarily used to verify argument parsing and standard I/O write capabilities.
...
A core responsibility of the shell is to provide an interface for the Virtual File System. The shell maintains a current working directory state (\texttt{current\_dir\_}) and provides utilities to navigate and inspect the hierarchy.
...
\begin{itemize}
    ...
    \item \textbf{ls [path]}: Lists the contents of a directory. It calls the \texttt{read\_directory} syscall to populate a buffer with \texttt{DirEntry} structures. The output logic includes syntax highlighting: directories are rendered in blue to visually distinguish them from regular files (Figure \ref{fig:shell_ls}).
    
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.9\textwidth]{res/shell_ls.png}
        \caption{Execution of the \texttt{ls} command showing the root directory structure.}
        \label{fig:shell_ls}
    \end{figure}

    \item \textbf{cat <file>}: Displays the contents of a file. It uses standard C library functions (\texttt{fopen}, \texttt{fread}) to open the target file and stream its content to \texttt{stdout} in 512-byte chunks. This verifies the kernel's pipe buffer management when transferring larger amounts of data (Figure \ref{fig:shell_cat}).
    
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.9\textwidth]{res/shell_cat.png}
...
The shell demonstrates the operating system's multitasking capabilities through commands that wrapper process management system calls.
...
The integration of Doomgeneric requires the implementation of a small set of C functions defined in \texttt{doomgeneric.h}. These functions serve as the bridge between the game engine and the kernel's system call interface.
...
In our implementation (\texttt{doomgeneric\_alkos.c}), these functions wrap the AlkOS system calls. For instance, \texttt{DG\_GetTicksMs} utilizes \texttt{GetClockValueSysCall(kProcTimePrecise)} to retrieve high-resolution timing, and \texttt{DG\_DrawFrame} invokes the \texttt{Blit} syscall to trigger framebuffer updates.
...
A naive integer scaling approach (e.g., simply multiplying pixels by an integer factor) used in this fork was not taking advantage of the available screen and resulted in wasted screen space. To address this, we restored back and integrated legacy Doom scaling algorithms that support \textbf{Squash} and \textbf{Stretch} modes.

We implemented an automatic mode selection algorithm in \texttt{doomgeneric.c}. Upon initialization, the game inspects the dimensions of the framebuffer allocated by the window manager and calculates the optimal scaling factor ($1\times$ to $5\times$) and mode (Stretch or Squash) to maximize screen usage.
\begin{itemize}
    ...
    \item \textbf{Special Case}: Resolution $800 \times 600$ trigger optimized path (e.g., $3\times$ squash with $50\%$ blending) to reduce visual artifacts.
\end{itemize}

\paragraph{Pixel Format}
The internal rendering engine of Doom operates in an 8-bit indexed color mode, utilizing a 256-color palette. However, the linear framebuffer provided by the AlkOS kernel operates in a true color mode. To bridge this gap, we implemented an efficient color conversion routine within the screen update loop. During the scaling process, each 8-bit pixel index is resolved against the current palette to obtain its RGB components. These components are then bit-shifted and packed according to the specific red, green, and blue field positions defined by the framebuffer metadata. This ensures correct color reproduction regardless of the specific pixel format utilized by the underlying hardware.

\paragraph{Buffer Optimization}
Doomgeneric renders the game to an internal buffer, and then we copy it again to the window manager's buffer. To maximize performance, we modified the \texttt{i\_video.c} rendering path to remove one layer of indirection. We allow the scaling routines to write directly into the framebuffer obtained via the \texttt{create\_graphic\_session} syscall. The scaling is performed in-place during the screen update phase.
...
To bridge this gap, we implemented a translation layer in \texttt{doomgeneric\_alkos.c}. The system maintains an array of \texttt{s\_PrevKeyStates} for all keys relevant to the game (movement, action, and menu keys). During every frame tick, the wrapper polls the current state of these keys using the kernel API. By comparing the current state with the previous state, we synthesize "Press" and "Release" events and push them into Doom's internal event queue.

We also implemented a lookup table to translate AlkOS \texttt{VirtualKey} enums (e.g., \texttt{VK\_LeftCtrl}) into Doom's internal key codes (e.g., \texttt{KEY\_FIRE}), ensuring correct mapping for gameplay actions.

\subsubsection{Porting Process}

The process of porting \textit{Doom} revealed several shortcomings in the existing AlkOS \texttt{libc} implementation, which required a subsequent extension of its functionality. Although the Doomgeneric fork is designed to be largely operating-system-agnostic, portions of its codebase implicitly assume the presence of a runtime environment with at least partial POSIX compliance. In particular, commonly used facilities such as string manipulation routines and directory handling interfaces were expected to be available. The following sections outline the key areas where the \texttt{libc} was enhanced to meet these requirements.
...
To support loading data from the file system, we expanded the `stdio` implementation to include \texttt{fopen}, \texttt{fread}, \texttt{fseek}, and \texttt{ftell}, backed by the underlying VFS file descriptor syscalls (\texttt{open}, \texttt{read}, \texttt{seek}). This allowed the Doom engine to seamlessly load WAD files from the root filesystem using standard C semantics. \\
