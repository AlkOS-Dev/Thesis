...
Precisely for the beforementioned reasons, implementing an operating system becomes such a rewarding task. Creating this apex software, deconstructing it into bits and pieces and untangling the problems it deals with, requires the highest standards of software engineering. In order to understand, why someone would one create an operating system from scratch as a Hobby - not for money, or a task at a job, or perspective of material gain - we'll bring a quote, the only one in this entire thesis: 
...
For these reasons an operating system is almost always created as an enterprise level initiative. Teams of seasoned profesionals are involved in working on it. Hobbyist OS'es created by one person or a handfull of people are on the other hand very few and far in between. 

To bring that into scale. We count the total number of pc games in tens of thousands, with the number of pc games on steam (the current biggest market for pc games) is estimated to be around ~121_000 as of early 2026 with 20k being made in 2025 alone. The number of mobile applications we count in milions, the number of apps on the play store (biggest app market for android) - around 2.180 mln with a similar number on the apple's equivalent store https://42matters.com/google-play-statistics-and-trends. The number of websites? We count that in billions, with 1.34 billion being the current estimate for the total number of websites on the internet https://www.internetlivestats.com/total-number-of-websites/. All are consumers of the operating system they are embedded in. What about OS'es?
What is listed here is by no means an exact number, since that is a really hard thing to estimate. It has to be taken into account that in the early era of computers, each new mainframe basically came with it's own OS. It will serve however as a rough estimate that'll allow to understand the scale of things.
The number of open source operating systems - mostly forks of linux and freebsd - is actively kept track of at https://distrowatch.com/search.php?status=All and totals 900+ entries. When talking about notable proprietary and historical OS'es,wikipedia lists around 250-300 operating systems on its page dedicated to them https://en.wikipedia.org/wiki/List_of_operating_systems.
Real time operating systems? Around 100-150 market leading ones https://en.wikipedia.org/wiki/Comparison_of_real-time_operating_systems.
The hobby ones created from scratch? An active list maintained at https://wiki.osdev.org/Projects suggests just around 175 projects as the current total.
[tldr, linux forks are plenty, systems made from scratch? proprietary and much less. Real time ssytems? same thing. Hobby OS'es? That's a really rare breed with only around 175 being there as of early 2026.]
Creation of one, from scratch, by three hobbyists, is therefore not an every day thing.

\subsection{Difficulties}

The number one difficulty in creation of one is the scarcity of quality educational materials and sources. This is a direct consuequence of the scale problem - too few people are involved in this for the paved paths to already be there. https://wiki.osdev.org/Expanded_Main_Page stands as the best source for hobby os'es. Creation of an OS without this resource would be practially impossible. Where do you start? what do you need to create and how? How does the hardware work? The intel manual doesn't tell you how to create an operating system for the cpu, in the same way as knowing how a motorcycle is constructed doesn't really tell you the whole story on how to drive it well. Perhaps the only site out there that can serve as that is the OS-dev.

The OS-dev site unfortunately suffers from a lack of contributors and broad articles that convey the idea but lack specifics. Articles that are outdated and sometimes (rarely) plain wrong and tutorials that stop not far after the operating system developers equivalent of "Hello world". 

One can of course read the source code of linux or minix, but this tasks proves very difficult for a beginner operating systems developer (even if otherwise a skillfull programmer). Without understanding what the components are for, what they achieve and what problems they solve, this task is more similar to reverse engineering than learning and understanding. Introduction into the problem space is a heavy difficulty in opearting systems. You either somehow know everything about them, or you know nothing and have a hard time enter this world. A gentle gradual introduction is non existent. 

There are of course very heavy technical difficulties that must be addressed, tasks of creating drivers, abstracting away hardware, creating optimized algorithms for memory management, task scheduling, file operations etc. But for someone that didn't have the experience of creatin an OS, and wanted to create one right now, the biggest problem would be exactly not knowing where to even start, and what's the theorey of, not of how a specific distribution works, but overall OS creation.

This document is partially about how we created our own operating system, which solutions we used, what problems we encountered and how we solved them and what came out of it [add here reference to the section our solution]. But it also is about introduction into the problem space of OS'dev and we wish for it to help as a reference for other developers trying their best to create their own OS [reference to label here about creating os ffrom scratch section]

\section{Scope of the Thesis}

The primary objective of this project, as initially defined, was to implement a functional scheduler capable of executing tasks within isolated address spaces. Specifically, the project aimed to achieve the following technical goals:
...
\begin{itemize} 
    ...
    \item Implementation of a basic scheduling algorithm -- specifically Round Robin. 
    \item x86\_64 Architecture -- targeting the AMD64 archietcture only. 
...
At the inception of it, the specific requirements were not rigidly defined, as the complexity and effort required to complete the implementation were uncertain.

\subsection{Functional Requirements}
\label{subsec:functional}

Based on the initial scope and architectural assumptions, the following functional requirements were derived:

\begin{itemize} 
    \item \textbf{x86\_64 Long Mode Transition} -- The kernel must successfully transition the CPU from 32-bit Protected Mode to 64-bit Long Mode, including the setup of a 64-bit Global Descriptor Table (GDT) and enabling 4-level paging.
    \item \textbf{Physical Memory Allocation} -- The kernel must implement a Physical Memory Manager (PMM) to track available RAM and provide a mechanism for allocating and freeing physical page frames based on the system memory map.
    \item \textbf{Virtual Memory Mapping} -- The system must implement a Virtual Memory Manager (VMM) and an MMU abstraction to map virtual addresses to physical frames, allowing for the creation of isolated address spaces.
    \item \textbf{Preemptive Scheduling} -- The kernel must provide a mechanism to forcibly interrupt a running task and switch context to another, ensuring that no single task can monopolize the CPU.
    \item \textbf{Context Management} -- The system must be able to create, save, and restore execution contexts (thread state including registers and stacks) to support multitasking.
    \item \textbf{Interrupt and Exception Handling} -- The kernel must configure the Interrupt Descriptor Table (IDT) and provide handlers for hardware interrupts (to drive the scheduler) and CPU exceptions (to ensure system stability).
\end{itemize}
...
The development process begins with the establishment of a cross-compilation toolchain targeting a generic, os-independent architecture (e.g., \texttt{x86\_64-elf}). Using a compiler provided by the host system may lead to issues, as such compilers typically assume that the generated code will execute within an operating system environment. These assumptions are invalid during development, where no such infrastructure exists initially.
...
This architectural decoupling ensures that the build logic remains agnostic of the underlying hardware. Retargeting the operating system to a new architecture (e.g., transitioning from \texttt{x86\_64} to \texttt{aarch64}) entails only the substitution of the active toolchain file, without requiring modifications to the project's build scripts or source code structure.
...
\begin{itemize} 
    \item File system abstraction layer 
    \item FAT filesystem implementation 
    \item Hardware abstraction layer 
    \item Userspace separation 
    \item Userspace shell 
    \item Chain loading
    \item ELF64 executable loading 
    \item Porting of external programs (Doom) 
    \item Thread sleeping mechanism 
    \item Wait queues implementation 
    \item System call mechanism 
    \item Extensive system call interface 
    \item Tickless kernel architecture 
    \item Thread termination capabilities 
    \item Thread creation 
    \item Process creation 
    \item Graphical output support 
    \item Basic window management 
    \item Page fault handling (lazy virtualization) 
\end{itemize}
...
\begin{itemize} 
    ...
    \item \textbf{Invariant TSC} -- To avoid implementing a driver for every possible AMD64 timer, the system relies on the Time Stamp Counter (TSC) as the primary clock source. The hardware must support an invariant TSC to ensure timing consistency regardless of frequency scaling.
...
The following section outlines the internal constraints established during development. These constants are hardcoded within the codebase, and multiple system mechanisms rely on them for correct operation.
