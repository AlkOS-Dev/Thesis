...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Colors
        cL4/.style={fill=violet!15, draw=violet!60!black, text=violet!30!black},
        cL3/.style={fill=blue!15, draw=blue!60!black, text=blue!30!black},
        cL2/.style={fill=teal!15, draw=teal!60!black, text=teal!30!black},
        cL1/.style={fill=orange!15, draw=orange!60!black, text=orange!30!black},
        cOff/.style={fill=gray!15, draw=gray!60!black, text=gray!30!black}
    ]

    % --- Address Bar ---
    \node[anchor=west, font=\small\bfseries] at (0, 1.2) {Virtual Address: \texttt{0x0000\_0080\_0020\_0000}};

    % Bar container
    \coordinate (barstart) at (0, 0);
    
    % L4
    \node[addrblock=1.8cm, cL4] (bar4) at (barstart) {\textbf{L4} [1]};
    \node[anchor=north, font=\tiny, text=gray] at (bar4.south) {47:39};
    
    % L3
    \node[addrblock=1.8cm, cL3] (bar3) at (bar4.east) {\textbf{L3} [0]};
    \node[anchor=north, font=\tiny, text=gray] at (bar3.south) {38:30};
    
    % L2
    \node[addrblock=1.8cm, cL2] (bar2) at (bar3.east) {\textbf{L2} [1]};
    \node[anchor=north, font=\tiny, text=gray] at (bar2.south) {29:21};
    
    % L1
    \node[addrblock=1.8cm, cL1] (bar1) at (bar2.east) {\textbf{L1} [0]};
    \node[anchor=north, font=\tiny, text=gray] at (bar1.south) {20:12};
    
    % Offset
    \node[addrblock=2.4cm, cOff] (barOff) at (bar1.east) {\textbf{Offset}};
    \node[anchor=north, font=\tiny, text=gray] at (barOff.south) {11:0};

    % --- Page Walk Flow ---
    
    \coordinate (flowStart) at (0.5, -2);
    \def\stepX{2.8}
    \def\stepY{-1.2}

    % CR3
    \node[draw=black!60, fill=gray!10, rounded corners, minimum width=1.2cm, minimum height=0.8cm] (cr3) at (0, -2.5) {CR3};
    \coordinate (cr3out) at (cr3.east);

    % PML4 Table
    \coordinate (pml4pos) at ($(cr3out) + (1.5, 0)$);
    \node[table] (pml4) at (pml4pos) {};
    \node[labelnode] at (pml4.north) {PML4};
    % Entries
    \draw[gray!30] (pml4.west) -- (pml4.east); % visual lines
    \node[entry, cL4] (pml4e) at ($(pml4.north) + (0, -0.5)$) {};
    \node at (pml4e) {\tiny Entry 1};
    
    % Link Bar to Table
    \draw[->, violet, dotted, thick] (bar4.south) to[out=-90, in=90] (pml4e.north);

    % PDPT Table
    \coordinate (pdptpos) at ($(pml4pos) + (\stepX, \stepY)$);
    \node[table] (pdpt) at (pdptpos) {};
    \node[labelnode] at (pdpt.north) {PDPT};
    \node[entry, cL3] (pdpte) at ($(pdpt.north) + (0, -0.9)$) {}; % Lower down
    \node at (pdpte) {\tiny Entry 0};
    
    \draw[->, blue, dotted, thick] (bar3.south) to[out=-90, in=90] (pdpte.north);

    % PD Table
    \coordinate (pdpos) at ($(pdptpos) + (\stepX, \stepY)$);
    \node[table] (pd) at (pdpos) {};
    \node[labelnode] at (pd.north) {PD};
    \node[entry, cL2] (pde) at ($(pd.north) + (0, -0.5)$) {};
    \node at (pde) {\tiny Entry 1};
    
    \draw[->, teal, dotted, thick] (bar2.south) to[out=-90, in=90] (pde.north);

    % PT Table
    \coordinate (ptpos) at ($(pdpos) + (\stepX, \stepY)$);
    \node[table] (pt) at (ptpos) {};
    \node[labelnode] at (pt.north) {PT};
    \node[entry, cL1] (pte) at ($(pt.north) + (0, -0.9)$) {};
    \node at (pte) {\tiny Entry 0};

    \draw[->, orange, dotted, thick] (bar1.south) to[out=-90, in=90] (pte.north);

    % CONNECTING ARROWS (The "Walk")
    \draw[->, thick, black!70] (cr3) -- (pml4.west);
    \draw[->, thick, black!70] (pml4e.east) -- ++(0.3, 0) |- ($(pdpt.north west)!0.2!(pdpt.west)$);
    \draw[->, thick, black!70] (pdpte.east) -- ++(0.3, 0) |- ($(pd.north west)!0.2!(pd.west)$);
    \draw[->, thick, black!70] (pde.east) -- ++(0.3, 0) |- ($(pt.north west)!0.2!(pt.west)$);
    \draw[->, thick, black!70] (pte.east) -- ++(0.5, 0) node[right, draw=black!50, fill=green!15, minimum size=1cm, align=center] (phys) {\textbf{Page}\\ \tiny 4KB Frame};

    % Offset arrow
    \draw[->, gray, dashed, thick] (barOff.south) to[out=-90, in=90] (phys.north);
