...
\section{Linux}
Family of operating systems working on Linux kernel is one of the most known in the world of operating systems, together with windows. The linux kernel is based on monolithic architecture. That means the range of responsibilities, the kernel has is wide and userspace is reduced to programs, which are only capable to obey the system and fully rely on it. In practices that means whole code managing memory, scheduling, interrupts and devices resides in kernel address space. Such approach has various pros and cons.

Advantages:
\begin{itemize}
    \item No-copy and memory efficiency -- as everything resides in single address space there is no need to copy the data itself between various address spaces a pointer is sufficient to pass the data between module.
    \item No context switch inside the kernel -- as we have single address-space if we enter the kernel we are able to simply invoke some function from different module rather than context-switching to some driver service.
    \item Easy source managment -- as we have single binary we only have to prepare single strong scheme for that binary and that is enought. Developing 20 services compiled separatly may lead to differing behaviour or data model between binaries easly.
    \item Easy implementation for small/mid size projects.
\end{itemize}

Disadvantages:
\begin{itemize}
    \item Stability -- some bug in some unused device's drivers may lead to system faulty
    \item Security -- some bug in some unused device's drivers may lead to exposing the kernel space address space to user space malicious executable
    \item Scales badly -- for huge projects, high standards must be ensured. As it is very easy to make the project messy and create unmaintainable net of references and dependencies.
\end{itemize}

To add some extensibility and prevent kernel recompilation to add drivers or modules linux introduces loadable kernel modules \cite{linux_lkm}. Those allow to dynamically extend the kernel code using well defined interface during the kernel.
...
\subsection{Timing}
Linux in older versions utilized periodic tick, that means the kernel was setting the timing devices to run the interrupt periodically in constant frequency e.g 1000hz. Such solution is very easy to implement and think about as we dont have to manage the event timeline at all. But it has some longstanding consequences. For example the constant frequency defines the granularity of sleep. As the solution is far from ideal linux introduced dynamic ticks utilizing high precision devices. That means the interrupts are scheduled based on the kernel needs and \texttt{hrtimer.h} infrastructure is utilized for that purpose \cite{linux_highres}\cite{linux_highres_code}. Such approach allows the cpu to be much more efficient or event disable the cores if there are no tasks to execute for them. But there is still some concept of \textbf{tick} mainly for performance and compatiblity matters, that means (not always in \texttt{CONFIG\_NO\_HZ} mode for single task there will be no interrupts) in most scenarios the time between the interrupts is capped to min(next\_event\_ns, tick\_ns). In other words hight resolution timer simulates periodic tick \cite{linux_highres}. When some does not need high precision (\texttt{nanosleep}) or quickly read the timer (without syscall, simply readonly mapped variable) this approach is ideal. Based on the kernel \textbf{tick} the \texttt{jiffies} variable is incremented \cite{linux_jiffies}. This is for example used for tcp connection timeouts to allows handling thousands of such connection, bookkeeping all of such event with high precision would be a real pain and waste of resources. For example \texttt{msleep} is working based on jiffies counter.

\subsection{Scheduler}
Linux introduces several scheduling policies \cite{linux_policies}\cite{linux_cfs} which operates based on priorities assigned to tasks. We have two major classes \textbf{Real Time} and \textbf{Fair}. Real time policies operates on priorities in range 1-99 and \textbf{Fair} tasks which operates only on 0 priority. Tasks with higher priorities are always scheduled before tasks with lower priorities. Linux provides 6 policy clasesses:

\begin{itemize}
    \item \texttt{SCHED\_DEADLINE} -- more important than any other policy and provides real time capabilities
    \item \texttt{SCHED\_FIFO} -- in given priority task has unlimited time and must leave the queue by himself - give the control back
    \item \texttt{SCHED\_RR} -- in given priority each task has constant time slice and after preempting is attachted to the end of queue
    \item \texttt{SCHED\_OTHER} -- 99\% of tasks, most of usual tasks resides here, managed by main algorithm
    \item \texttt{SCHED\_BATCH} -- less interactive tasks, runs longer without context switches, thus need to wait longer before being executed again
    \item \texttt{SCHED\_IDLE} -- the most important tasks runs only when there is nothing else to do
\end{itemize}

Starting from versions 2.6.23 Linux \cite{linux_cfs} up to 6.6 \cite{linux_eevdf} the \texttt{CFS} (Completly Fair Scheduler)  was the architecture of linux scheduling. Starting from 6.6 the \texttt{EEVDF} (Earliest Eligible Virtual Deadline First) started to replace the CFS architecture. Those algorithms are used for \textbf{Fair} tasks class. We will briefly describe both of those.

\subsubsection{Completly Fair Scheduler}
Linux documentation describes \textbf{CFS} quite accuratly:
\begin{quote}
...
The implementation uses red-black tree sorted by the virtual runtime representing the execution timeline. The next task is picked as a leftmost tasks from the tree (less time on CPU spent) and runs until the second leftmost task becomes the actual min task with some minimal granularity to prevent over-scheduling.

\subsubsection{Earliest Eligible Virtual Deadline First Scheduler}
This is becoming the new version of linux scheduling and is addressing issues of previous \textbf{CFS} implementation. New approach works similarly as \textbf{CFS} but operates on deadlines instead of runtime \cite{linux_eevdf}. That allows latency-sensitive tasks to be prioritized.

\subsection{Memory Management}
\label{subsec:linux_mem}
Linux has centralized memory management with various APIs available: \texttt{kmalloc}, \texttt{kzalloc}, \texttt{vmalloc}, \texttt{kvmalloc} and many more \cite{linux_mem_interfaces}. 

\subsubsection{Physical Memory}
As linux is architecture independant kernel it abstracts the architecture detail. The physical memory is partitioned into zones each of them with different purpose \cite{linux_mem_phys}:

\begin{itemize}
    \item \texttt{ZOME\_DMA} -- represents memory suitable for DMA (Direct Memory Access) by devices that cannot access all addressable memory.
    \item \texttt{ZONE\_NORMAL} -- normal memory accessible by the kernel all the time
    \item \texttt{ZONE\_MOVABLE} -- normal accessible memory as \texttt{ZONE\_NORMAL} but their content may be moved between physical pages while not changing the virtual address
    \item \texttt{ZONE\_DEVICE} -- memory reserved for devices such as GPUs and maps to device drivers
    \item \texttt{ZONE\_HIGHMEM} -- not covered by permanent mappings. Not used frequently mostly available on 32-bit architectures. 
\end{itemize}

To enchance the performance of multi core system linux takes two-step strategy \cite{linux_mem_phys}. There is one global buddy allocator \ref{buddy_allocator} operting with locks and full synchronization and \texttt{PCP} (Per-CPU Pagessets) with stores free ares assigned to specific core. First we allocate from local PCP and afterwards we fallback to global allocator.

\subsubsection{Virtual Memory}

Each process has own Page Table which is responsible for mapping the virtual address to physical one. The physical space is acquired from Physical Memory managmenet \ref{subsec:linux_mem}. Then managing the virtual addresses is done per each process utilizing red-black trees for fast lookup for free areas. The kernel address space is mapped to each process. Additionally linux implement on demand paging, that is lazy approach alloation physical pages only when the process actually needs them utilizing page-fault mechanism.

\subsection{System Interface}

Linux is a kernel from family of unix-like operating systems. The main communication with system goes through system calls and glibc which provides wrappers around those calls. 

\subsubsection{Standards}

Linux implements \textbf{POSIX} standard which includes functions like \texttt{fork()}, \texttt{exec()} or concept of \texttt{pthread}.

\subsubsection{Fork mechanism}

\texttt{fork()} mechanism comes from historical decisions. The idea to copy everything from parrent process sounds like terrible idea and in reality may introduce performans issues. That makes this approach quie hard to implement as it must be extremely efficient in real world scenarios. Especcialy considering that 99.9\% cases after \texttt{fork()} the \texttt{exec()} function is called immidiently erasing mostly everything from previous process. Because of that Linux implemented various optimizations to not make it into a bottleneck, including \texttt{COW} mechanism. To sum up fork has some advantages and disadvantages:

\noindent Pros of implementing fork mechanism:
\begin{itemize}
    \item Simple implementation of pipes and pipelining
    \item Easier state propagation, no need to specify files or streams manually
\end{itemize}

\noindent Disadvantages:
\begin{itemize}
    \item Hard to implement efficiently - requires multiple optimizations
    \item Hard to implement overall as requires copying every structure 
    \item Still might be slow in some edge cases - big page tables
    \item Over-copying and puts some pressure on programmer to release the resources before exec
    \item Concept hard to understand for begginers
    \item Not portable to archtiectures without MMU
\end{itemize}
