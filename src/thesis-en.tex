...
The choice of allocation algorithm fundamentally shapes the PMM's performance profile. Before examining specific algorithms, it is essential to deeply understand the core problem: \textbf{fragmentation}---the inability to reuse memory that is free. This occurs when free blocks exist but are too small or poorly positioned to satisfy requests.

\textbf{Internal vs.\ External Fragmentation.} Operating system developers must distinguish between two forms. \textit{Internal fragmentation} arises when allocated blocks are larger than requested due to size-class rounding (e.g., a 50-byte request rounded to 64 bytes wastes 14 bytes inside the block). \textit{External fragmentation} occurs when free memory is scattered into many small holes, none large enough to satisfy a request, even though total free memory exceeds the request size. The lower bound on worst-case external fragmentation for any allocator is $O(M \cdot \log n)$, where $M$ is live data and $n$ is the ratio between largest and smallest block sizes~\cite{wilson1995survey}.
...
\textbf{Placement Policies.} Allocators implement placement policies that determine where to allocate requested blocks:
\begin{itemize}
    \item \textbf{First Fit:} Use the first sufficiently large free block (address-ordered). Fast, and tends to pack one end of memory.
    \item \textbf{Best Fit:} Use the smallest block that is large enough. Minimizes wasted remainder but may accumulate unusable ``splinters.''
...
\textbf{Splitting and Coalescing.} Two subsidiary mechanisms support placement policies. \textit{Splitting} divides a large block to satisfy a smaller request, leaving the remainder as a new free block. \textit{Coalescing} merges adjacent free blocks into a larger one. General coalescing requires knowing neighbor boundaries---this is supported by \textit{boundary tags}~\cite{knuth1973art}: each block stores its size in both a header and a footer, enabling $O(1)$ lookup of neighbors. A space optimization avoids footers on allocated blocks by stealing a bit from the next block's header to indicate whether the previous block is free.

Allocator design involves three levels of abstraction: a \textbf{strategy} that attempts to exploit regularities in program behavior, a \textbf{policy} that determines exactly where to place blocks, and a \textbf{mechanism} that implements the policy efficiently. The following subsections examine the dominant mechanisms and their trade-offs.
...
The Buddy System bridges the gap between the contiguity support of bitmaps and the speed of free lists. It is the standard algorithm in general-purpose kernels~\cite{knuth1973art}, though it comes with significant trade-offs that must be understood.
...
The slab allocator also exploits \textbf{cache coloring}. Hardware caches are indexed by address, so objects at the same offset within different slabs contend for the same cache lines. By offsetting object start addresses within slabs (varying the ``color''), the allocator spreads objects across more cache lines, reducing conflict misses and improving bus utilization.
...
For AlkOS, a straightforward first step is per-CPU quick lists for single-page allocations, progressing to full slab caching for high-traffic object types.
