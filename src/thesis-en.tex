...
The Physical Memory Manager (PMM) forms the foundational layer of any operating system's memory subsystem. It is responsible for tracking, allocating, and reclaiming the machine's finite physical RAM---measured in discrete units called \textit{page frames} (typically 4~KiB on x86-64). Unlike higher-level allocators that operate on virtual addresses, the PMM deals directly with physical hardware constraints.
...
The choice of allocation algorithm fundamentally shapes the PMM's performance profile. Before examining specific algorithms, it is useful to understand the core problem: \textbf{fragmentation}---the accumulation of scattered, unusable holes in memory. When blocks are allocated and freed in varying sizes and orders, the remaining free space may become fragmented into pieces too small to satisfy future requests, even if total free memory is abundant. Operating system developers must distinguish between \textit{internal fragmentation} (wasted space within allocated blocks due to size rounding) and \textit{external fragmentation} (unusable gaps between allocated blocks). A thorough understanding of fragmentation causes and mitigations is essential for informed allocator selection~\cite{wilson1995survey}.

Allocator design involves three levels of abstraction: a \textbf{strategy} that attempts to exploit regularities in program behavior, a \textbf{policy} that determines exactly where to place blocks, and a \textbf{mechanism} that implements the policy efficiently. Most real programs exhibit characteristic memory usage patterns---gradual accumulation (ramps), bursty peaks, or stable plateaus---rather than the random behavior assumed by many theoretical analyses. Allocators that exploit these patterns (e.g., placing temporally adjacent allocations in contiguous memory) can significantly reduce fragmentation~\cite{wilson1995survey}. The following subsections examine the dominant algorithmic approaches.
...
This approach offers strict $O(1)$ performance for single-page operations. Additionally, the LIFO behavior creates cache locality: recently freed pages (likely still in CPU cache) are reallocated first. However, the stack cannot efficiently satisfy requests for contiguous multi-page allocations, as the order of pages on the stack reflects deallocation history, not physical adjacency.

\paragraph{The Buddy System}

The Buddy System bridges the gap between the contiguity support of bitmaps and the speed of free lists. It is the standard algorithm in general-purpose kernels~\cite{knuth1973art}.
...
The primary drawback is \textbf{internal fragmentation}: a request for 33~KiB requires a 64~KiB block, wasting approximately 48\% of the allocation.

\paragraph{Two-Level Segregated Fit (TLSF)}

For real-time systems where deterministic latency is critical, the TLSF allocator guarantees strict $O(1)$ worst-case execution time for both allocation and deallocation, without the internal fragmentation penalty of power-of-two rounding.
...
Physical memory is not always uniformly addressable by all system components. Hardware constraints force the PMM to consider \textit{which} physical frames can satisfy a given allocation:
...
\begin{itemize}
    \item \textbf{Object Caching (Slab Allocator):} For frequently allocated objects of fixed size (e.g., inodes, task structures), repeatedly invoking the PMM and initializing objects is wasteful. The \textit{slab allocator} pre-allocates slabs of objects and caches them, retaining constructor state between allocations. This dramatically reduces allocation latency for hot paths~\cite{bonwick1994slab}.
    
    \item \textbf{Multiprocessor Scalability:} On SMP systems, a single allocator lock becomes a contention bottleneck. Solutions include per-CPU caches, per-CPU free lists, and architectures like Hoard that use per-processor heaps with global heap fallback~\cite{berger2000hoard}. AlkOS can achieve basic scalability by maintaining per-CPU quick lists for common allocation sizes.
    
    \item \textbf{Deferred Coalescing:} Aggressively merging freed blocks only to split them again wastes CPU cycles. Many allocators defer coalescing, keeping recently-freed blocks on fast ``quick lists'' and only merging them during memory pressure or periodic sweeps~\cite{wilson1995survey}.
\end{itemize}
