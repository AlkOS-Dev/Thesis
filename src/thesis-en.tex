...
Probably first thing that must be done after running any code on the target architecture is to establish some communication with world. For emulation environment one may use some devices to communicate with emulating framework (e.g. qemu uses serial port for communication and it may be attachted to linux shell session) for real devices user may have to display some fonts on the screen (e.g. on amd64 desktop platforms VGA may be easly used) or even implement some basic net stack. The preffered communicatin type should allow two-way communication on early-stages to allow some testing and providing input to the kernel. This mostly will be required for debug purposes and testing and as the development goes further it will be reasonable to disable the communication and associated debug traces by some compilation flags. It is important to separate debug only communication from real output devices as it must be disabled for release builds. 
...
As we can see on \ref{fig:qemu_comms} alongisde typical communication on the screen together with keyboard the kernel outputs some text to the qemu serial-port, which then streams that to the shell. Such approach allows some host script to quickly parse the logs and detect some bugs or failures or manually inspect what happend before the failure.

\subsection{Memory Preloading and Discovery}
TODO: Kryczka opisz multiboot2 przykladowo i zbieranie informacji o zarezerowowanych przestrzeniach

\subsection{Enabling Interrupts and Exceptions}
\label{subsec:os-tutorial-interrupts}

Before proceeding to implementation of memory \ref{subsec:os_tutorial_mem} its quite important to establish at least some basic interrupt handling. On most platforms exception system relies fully on interrupt mappings. That means without them we are not able to display any debug information on desired communication device.

Exception Interrupt handlers should display some descriptive message in case of failures. Such message must contain the CPU state, source of the exception, location where it occurs (which instruction caused the exception) and possibly why it happend (might be implemented as error code). Example kernel dump on exception might look like this:
...
Before initializing Memory Management that is actually everything we need from interrupts, as we will probalby dont have any use case for interrupt driver devices without booting up the consumers (processes) or working with multiple cores (that also needs memory for bookeeping structures). But we will describe what we need from interrupts and what we should implement for that anyway.

\subsubsection{Design considerations}

It is important to note that low-level hardware complying interrupt handlers usually differs strongly from usual compiled functions (requires special instruction on begining or end) and thus implementing multiple complying function to swap on runtime might be difficult. The good idea is probably to create low-level wrapper performing all needed architecture dependandt operations efficiently and invoking high level function performing actual kernel's work. It still has some drawbacks as we will not be able to swtich that high-level function as it will be compiled into the low-level handler. To address that problem we need some good hardware abstraction which will be a middle man between the architecture wrapper and kernel high-level function. This abstraction should perform operations on low-level table and allow swapping the functions on runtime, this might be done by simple array of abstracted handlers mapped to hardware interrupts.Also we will be mapping devices to hardware interrupts dynamically during runtime based on user configiration and discovery and we will have to swap drivers and logic anyway. Without this modularity we will have to struggle either with multiple stages of intterrupts init, implementing multiple handlers on architecture side or writing incredibly complex handlers quering the kernel if we already enabled some functionality or not (e.g. page fault handler would have to check if we already handle virtualization or we did something stupid).

It is also very important to design the handlers to perform as little work as possible. That excludes any waiting, sleeping or tracing. If device handling requires some more complex work or reacting on the change we may need to invoke the scheduler and perofrm context switch while exiting the interrupt to perform necessary actions. But we should avoid running this logic inside the interrupt as we are blocking any other interrupts while doing that and we may lose some informations during that process. That gives us another conclusion that we also have to consider how to switch tasks within interrupt system, it may be done in the abstraction.

While implementing the abstraction we can assume easly these three classes of interrupts:
\begin{itemize}
...
Some architectures (for on example amd64 we have old PIC \cite{osdev-pic} and improved SMP supprting APIC \cite{osdev-apic}) we should also take that into account when designing the abstraction. And finally when we you want to enable SMP and utilize multiple cores the interurpt mechanism will be a major way of communication between the cores. 

\subsection{Tracing System}

As we learned from \ref{subsec:os-tutorial-interrupts} in some situations we should not trace anything at all, but in general case we want to have some kernel log and dump messages to file or terminal. The problem here is that writing to a file or any device at all takes a lot of time and our system code should work as fast as possible. To solve this we may want to design tracing framework which is robust (will work properly in concurrent environment, interrupts + SMP) and allows to fast writing the traces bypassing immediate writes to devices. It may be done by allocating big cyclic buffor for the messages and dumping it using special dumping task working periodically. 
