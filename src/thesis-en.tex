...
\begin{figure}[htbp]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}
            ...
            \draw [fill=white, thick, rounded corners=3.0] (0.5, 4.5) rectangle (13.5, 6.5);t
...
Internal memory allocation for kernel data structures and VM metadata is performed using a slab allocator. Minix 3 supports demand paging: when a process attempts to access an unmapped virtual address, a page fault is raised. The kernel intercepts this exception and forwards the fault information to the VM server, which resolves it by mapping an appropriate physical frame or retrieving the required page from secondary storage \cite{shenoy_lecture}.
