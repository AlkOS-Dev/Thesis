...
\begin{itemize} 
    ...
    \item \textbf{Memory Management Unit (MMU)} -- The kernel relies on a standard Memory Management Unit (MMU). It assumes that the hardware enforces standard paging mechanics and does not support non-standard extensions or software-managed TLBs.
    \item \textbf{Physical Memory Layout} -- The system expects a standard, flat physical memory map provided by the bootloader (via Multiboot2). It does not support tiered memory architectures, NUMA (Non-Uniform Memory Access) optimizations, or memory hot-plugging.
    \item \textbf{Linear Framebuffer} -- Graphics support relies entirely on a pre-initialized linear framebuffer provided by the system firmware (VBE or UEFI GOP). Use of hardware acceleration or specific GPU drivers is not supported; all rendering operations are software-based.
