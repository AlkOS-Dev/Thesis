...
In simple embedded architectures (e.g., microcontrollers used in automotive braking systems or household appliances), the entire application code is often stored in this non-volatile memory. The memory controller maps this storage directly into the CPU's addressable space. This technique, known as \textbf{Execute In Place (XIP)}, allows the CPU to fetch and execute the developer's code from the very first clock cycle. The developer "owns" the machine from the first nanosecond.
...
\subsubsection{The Complexity of the x86 Platform}
...
\subsubsection{The Role of Firmware and Bootloaders}
By the time a modern Operating System kernel begins execution, it is likely the fourth or fifth program in the boot chain. The entity responsible for handing over control to the OS is the system Firmware (Legacy BIOS or UEFI).

The firmware's responsibility is to abstract the diverse hardware implementations of different motherboards and provide a mechanism to load an OS from a disk. However, relying solely on firmware is insufficient for a portable operating system for several reasons:
\begin{itemize}
    \item \textbf{Inconsistent State:} The state of the CPU (interrupts, paging, modes) varies significantly between BIOS and UEFI.
    \item \textbf{Feature Limitations:} Firmware often leaves the CPU in a conservative state (e.g., 16-bit Real Mode or 32-bit Protected Mode) with advanced features disabled.
    \item \textbf{Format Differences:} BIOS loads raw sectors, while UEFI understands file systems.
\end{itemize}

To solve this, a third-party \textbf{Bootloader} (such as GRUB or Limine) is utilized. The bootloader normalizes the environment, abstracting away the underlying firmware differences, and loads the kernel file into memory.

\subsubsection{The OS-Level Bootloader}
Even with a standardized bootloader, the kernel cannot assume full control immediately. The generic bootloader cannot know the specific requirements of the OS kernel. For instance:
\begin{itemize}
    \item The kernel may require the memory to be mapped to the higher half of the virtual address space.
    \item Floating-point units (FPU, SSE, AVX) are typically disabled at boot and must be explicitly enabled; attempting to use them without initialization results in hardware exceptions.
    \item The OS must define its own Global Descriptor Table (GDT) and Interrupt Descriptor Table (IDT) to enforce its specific security model and segment access rights.
\end{itemize}

Therefore, a robust operating system must implement its own initialization stage—effectively an \textbf{OS-Level Bootloader} or "Trampoline." This architecture-specific code is responsible for taking the machine from the normalized state provided by the external bootloader, enabling the specific CPU features required by the kernel (Virtualization extensions, AVX, Paging modes), and establishing the runtime environment (Stack, Virtual Memory) before passing control to the architecture-agnostic kernel main function.
...
\section{Bootloader}
\todo[inline]{Loader32 and Loader64 are very simplified here}
