...
Finally having all previous components implemented we can proceed to designing and implementing our scheduling components. First we have to define structures for our tasks. We will start with a concept of Process.
...
When designing a scheduler or any internal kernel code we must be aware what type of synchronization do we need. Additionally we always should keep in mind that synchronization must be also efficient so we should guard only essentials.

\begin{itemize}
    \item Interrupt Synchronization - if we are implementing preemptive scheduler interrupt may change the context at any point. If not we also should consider this type of synchronization. If interrupt may execute same code as syscall  execution or kernel thread then we are still faced with races.
    \item SMP Synchronization - if multiple cores may access the data simultaneously we must be guared from simultaneous access. For simple critical sections we may use primitives like barriers \cite{barriers} or spinlocks \cite{spinlock}. But for more complicated scenarios we still needs some mutex-like mechanisms for that we must use blocking locks, which will utilize wait queues \ref{subsec:wait_queue}.
\end{itemize}

\subsubsection{Scheduler}

Having thread and process management done last thing to implement we need some machinery to pick best suitable task to run next on preemption or during yielding. And that is the main responsiblity of the scheduler to track the execution of the threads and always pick most suitable tasks to run. The difficulty of scheduler implementation will mostly depend on which approach we are working with \ref{sec:scheduling_approaches}.

The simplest option to implement is single core non-preemptive (cooperative) scheduler. In this case we dont have to worry about synchronization inside the scheduler at all as only scenario its internals are invoked is on yield (we have single core and no preemption). Also sleeping mechanism my be simplified here, as the task cannot be preempted we could only wake up tasks during yield calls and dont bother with timing at all.

Much more difficult to implement is preemptive scheduler as we are now additionally responsible for how to distribute the cpu time with bigger flexibility, the latency depends on us in bigger scale and we have to guard oursefles from interrupts that may change the context at any point. The scheduler also should be more precise for sleeping mechanism and ensure to execute sleeping tasks more accuratly.

Finally implementing the scheduling for SMP systems will not change the difficulty drastically compare to preemptive and cooperative gap. The only new challange here is to ensure synchronization is done properly between the cores. Spinlocks are sufficient here. We can additionally improve the performance by introducing core-local schedulers and some global load balancer mechanism instead of single global scheduler. In this case we dont need any smp synchronization inside the scheduler, we only need synchronized task migration protocol between the cores if some core is under or orver utilized.

First we have to decide on scheduling algorithms we will use. We may want to optimize our scheduler in various ares, which mostly include:

\begin{itemize}
    \item minimazing latency
    \item minimzaing wait time
    \item maximizing throughput
    \item maximizing fairness
\end{itemize}

We cannot make a scheduler which is ideal in all of this fields, instead we should find a good enough solution for our main purpose. Secondly the scheduler should prioritize the tasks as some things are more important than others. For example sound driver or block device driver threads may need to be executed before other only because to ensure consitency for the users. Some solutions introduce cooperative policies for that case (For example \texttt{SCHED\_FIFO} from linux \ref{subsec:linux-scheduler}). Generally it is good idea to allow the tasks to work on cooperative mode only if there is no more important task to do. It is crucial for some use cases like video graphics or sound drivers to deliver the results no matter what else we do, as it directly maps on user the experience. 

To present the concept on example we will show simplest  preemptive scheduler possible: that is periodic single core round robin scheduler:

We have to maintain single queue of tasks to execute. New tasks are added to the end, and picked up from the front. Each constatn time (e.g. 10ms) the task is interrupted, added to the end of queue and next task is picked from the fron. In pseudo code it may look like that:
...
void TimerHandler() {TODO: JAKUB
    InitializeTimingModule(TimerHandler, SCHED_PERIOD_TICK);
}
\end{lstlisting}

It may be even enough for SMP system as core local scheduler, assuming that load balancing could take some tasks from local tasks field.

\subsubsection{Blocking - Wait Queues}
\label{subsec:wait_queue}

No matter which type of scheduler we implementing, we need some mechanism to block the thread and deffer the execution unil some event happens. In reality it might be a sleeping lock, thread joining or any other type of synchronization. For this case we need to sustain multiple queues distributed all over the code. In contrast to scheduler queues which are mostly centralized (either core local or globlal). To properly distinguish both cases the task state might be handy solution here to create special state \texttt{BLOCKED} reserved for tasks waiting on some wait-queue. In case of thread removal we will easy know how to cleanup the structures. Here also intrusive structures come in handy with removal property \ref{subsec:intrusive}. The holder of queue is responsible for waking up all the threads waiting on the event (that is moving thread from its wait-queue to scheduler queues) on the other hand usually the task's code itself will put him inside such a queue on some synchronization point. 
