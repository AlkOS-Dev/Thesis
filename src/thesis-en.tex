...
The strategy is the high-level philosophy used to combat fragmentation. It relies on heuristics about program behavior. For example, a common strategy is to "cluster objects that will likely die at the same time." If a specific system task allocates 10 small objects, processes them, and frees them instantly, a good strategy tries to keep those objects adjacent in physical memory so that when they are freed, they combine to form a single large free block rather than 10 small holes.
...
To solve the contiguity problem while maintaining decent performance, general-purpose operating systems (like Linux) utilize the \textbf{Buddy System}. The Buddy System is a specific implementation of a broader class of allocators known as \textbf{Segregated Fits}.
...
The choice of allocation algorithm fundamentally shapes the PMM's performance profile. 

\subsection{Fragmentation}
Before examining specific algorithms, it is essential to deeply understand the core problem: \textbf{fragmentation} - the inability to reuse memory that is free. This occurs when free regions exist but are too small or poorly positioned to satisfy requests.
...
\subsection{Program Behavior Patterns}
In search of solutions to the above problems, we look into the following:
...
\begin{itemize}
    ...
    \item \textbf{FIFO (First-In-First-Out):} Pages ``age'' before reuse, giving the system time to reclaim them efficiently (e.g., for write-back). FIFO ordering can reduce fragmentation compared to pure LIFO, performing nearly as well as address ordering~\cite{wilson1995survey}.
\end{itemize}
