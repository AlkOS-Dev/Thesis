...
\renewcommand{\title}{Functional operating system kernel: AlkOS}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Chapter 3 --- Analysis of Existing Solutions:
        \begin{itemize}
        ...
        \item Chapter \ref{chap:high_level_subsystems} --- High-Level Subsystems:
        \begin{itemize}
    ...
    \item \textbf{Tooling and Quality Assurance:}
        \begin{itemize}
            ...
            \item Development of the project's automation infrastructure (the majority of Bash scripts under \texttt{./scripts}).
        \end{itemize}
\end{itemize}
...
\begin{itemize} 
    ...
    \begin{itemize}
        \item Chapter \ref{chap:os_from_scratch} --- Creating an Operating System from Scratch:
        \begin{itemize}
            \item Section \ref{subsec:theory_toolchain} --- Cross-Compilation Toolchain
            \item Subsection \ref{subsec:building_machinery} --- Building Machinery
            \item Section \ref{subsec:refcounting} --- Reference Counting
            \item Section \ref{subsec:theory_std} --- Implementation of the Standard Library
            \item Section \ref{sec:theory_fs} --- File Systems
            \item Subsection \ref{subsubsec:syscalls} --- Syscalls
            \item Subsection \ref{subsec:libc_headers} --- Libc System Headers
            \item Section \ref{sec:host_env} --- Host Environment --- Working OS
        \end{itemize}

        \item Chapter 3 --- Analysis of Existing Solutions:
        \begin{itemize}
            \item Section \ref{sec:minix-research} --- Minix 3 Research and Analysis.
        \end{itemize}

        \item Chapter \ref{chap:high_level_subsystems} --- High-Level Subsystems:
        \begin{itemize}
            \item Section \ref{section:fs} --- File System
            \item Subsection \ref{subsec:policiesOverview} --- MLFQ Scheduling Policy
        \end{itemize}

        \item Chapter \ref{chap:users_manual} --- User's Manual
    \end{itemize}
    ...
    \item \textbf{Data Structures and Standard Library:}
    \begin{itemize}
        \item Implementation of core C standard library components (\texttt{libc}), including C runtime startup (\texttt{crt0}), string manipulation (\texttt{strcpy}, \texttt{strlen}, \texttt{strcat}, \dots), formatted output (\texttt{snprintf}), dynamic memory management (\texttt{malloc}, \texttt{free}), and character classification (\texttt{ctype.h}).
        \item Implementation of selected C++ standard library components (\texttt{libc++}), including \texttt{std::span}, \texttt{std::string\_view}, and \texttt{std::atomic}.
        \item Design and implementation of core kernel data structures: \texttt{CritBitTree} (efficient string lookup), \texttt{TaggedPtr} (pointer-embedded metadata), \texttt{RefCounter} (intrusive reference counting), \texttt{BitField} (platform-safe bit-field), and \texttt{LruCache} (fixed-capacity cache with eviction policy).
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Thesis Documentation and Analysis:}
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                \item Section \ref{sec:mem_discovery} --- Memory Preloading and Discovery.
                \item Section \ref{subsubsec:physical_memory_management} --- Physical Memory Management Theory (Fragmentation, Allocators).
                \item Section \ref{sec:vmm} --- Virtual Memory Management Theory (Hardware, Architecture, Algorithms, Locality).
                \item Section \ref{sec:screen_drawing_theory} --- Screen Drawing Theory.
            \end{itemize}
            \item Chapter \ref{chap:low_level_implementation} --- Low-Level Kernel Implementation:
            \begin{itemize}
    ...
    \item \textbf{Core Kernel Infrastructure:}
        \begin{itemize}
            \item \textbf{Bootloader:} Design and implementation of the two-stage internal bootloader (Loader32 and Loader64), ELF64 loading, and mode transitions (Protected to Long Mode).
            \item \textbf{Memory Management:} Implementation of the Physical Memory Manager (Bitmap and Buddy allocators), Slab Allocator for kernel objects, and the Virtual Memory Manager (Paging, recursive mapping, demand paging, and page fault handling).
            \item \textbf{Hardware Abstraction:} Implementation of the HAL interface.
        \end{itemize}

    \item \textbf{User Space and Graphics:}
        \begin{itemize}
            \item \textbf{Graphics Subsystem:} Linear framebuffer drivers, software rasterizer, font rendering, and the Window Manager.
            \item \textbf{Input Subsystem:} PS/2 Keyboard driver and input event routing.
            \item \textbf{User Space Environment:} ELF executable loader, implementation of the System Shell (before porting it to user space), and GUI enablement.
        \end{itemize}
    ...
    \item \textbf{Project Infrastructure:}
        \begin{itemize}
            \item Architecture of the CMake-based build system.
            \item Configuration of the Cross-Compilation Toolchain.
            \item Setup of host-based tests.
            \item Setup of CI/CD pipelines and development scripts.
        \end{itemize}
\end{itemize}
...
\label{subsec:building_machinery}
...
In academic and hobbyist operating system projects, simpler file systems such as FAT are frequently preferred over more sophisticated journaling file systems. FAT offers broad host-side tool support, simplifying inspection and debugging, and its relatively simple on-disk structures reduce driver implementation complexity.
...
These counters track the numbers of live threads and threads awaiting cleanup which are used during process termination to determine when the process can be fully deallocated.
...
\textbf{Thread Ripper} --- responsible for the deallocation of thread resources, including stacks.
...
\begin{itemize}
    ...
    \item \textbf{exec\_async <file> (or ./<file> \&)}: The shell does not wait for the child process, allowing the user to continue issuing commands while the background process executes. This validates the scheduler's ability to timeshare the CPU between the interactive shell and compute-intensive background tasks.
...
\begin{itemize}
    ...
    \item \textbf{Special Case}: Resolution $800 \times 600$ triggers optimized path ($3\times$ squash with $50\%$ pixel blending) to reduce visual artifacts.
\end{itemize}
...
Doom expects a queue of input events (key presses and releases) using its own internal key codes. AlkOS, however, exposes a polling-based interface via the \texttt{GetKeyState} syscall, which returns the instantaneous state of a \texttt{VirtualKey} (keyboard layout agnostic key code).
...
\begin{itemize} 
    ...
    \item \textbf{Abstract File Systems} -- Add pseudo file systems (e.g., \texttt{procfs}, \texttt{sysfs}) for extensible system interfaces.
    ...
    \item \textbf{Fast System Call Mechanism} -- Replace the current interrupt-based syscall mechanism with the \texttt{syscall}/\texttt{sysret} instructions to reduce overhead and improve kernel entry performance.
