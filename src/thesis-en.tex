...
For kernels utilizing a periodic tick, handling sleep operations with granularity aligned to the tick interval is straightforward. Waking upt tasks can be performed on every timer interrupt. We just have to count how many times the interrupt fired (how many ticks have passed). Efficient algorithms, such as the timer wheel \cite{linux_timer_wheel}, exist to manage this type of coarse-grained sleeping.
