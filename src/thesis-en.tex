...
To resolve these architectural limitations, modern general-purpose operating systems implement Virtual Memory (VM). Fundamentally, Virtual Memory is not merely a technique for expanding available RAM via secondary storage (swapping), but rather an abstraction layer that decouples the \textit{logical view} of memory from the \textit{physical reality} of the hardware. 
...
Although many commercial operating systems utilize this abstraction to overcommit memory by swapping pages to a hard disk (a mechanism historically associated with the term Virtual Memory), the primary focus in kernel design remains on the protection and organization capabilities of the architecture. Even in the absence of swap, the VMM is essential for:
\begin{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        \node[draw, minimum width=3cm, minimum height=1cm] (cr3) {Root Register};
        \node[draw, minimum width=2cm, minimum height=3cm, right=2cm of cr3] (l4) {Level 4 Table};
        \node[draw, minimum width=2cm, minimum height=3cm, right=1cm of l4] (l3) {Level 3 Table};
        \node[draw, minimum width=2cm, minimum height=3cm, right=1cm of l3] (l2) {Level 2 Table};
        \node[draw, minimum width=2cm, minimum height=3cm, right=1cm of l2] (l1) {Level 1 Table};
        \node[draw, minimum width=2cm, minimum height=1cm, right=1cm of l1] (phys) {Physical Frame};
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        % High Level Block
        \node[draw, fill=blue!10, minimum width=6cm, minimum height=1.5cm, rounded corners] (mi) at (0, 2) {\textbf{Machine Independent (MI)}};
        \node[below=0.1cm] at (mi.north) {\scriptsize Manage Regions, Trees, Policies};
        
        % Interface
        \node[draw, dashed, minimum width=6cm] (api) at (0, 0.8) {PMap Interface};
        
        % Low Level Block
        \node[draw, fill=green!10, minimum width=6cm, minimum height=1.5cm, rounded corners] (md) at (0, -0.5) {\textbf{Machine Dependent (MD)}};
        \node[below=0.1cm] at (md.north) {\scriptsize Manage Page Tables, TLB, Control Regs};
        
        % Hardware
        \node[draw, fill=gray!20, minimum width=4cm, minimum height=1cm] (hw) at (0, -2.5) {Hardware (MMU)};
        ...
        % Labels
        \node[right=3.2cm, align=left] at (mi) {Logical View\\(Regions)};
        \node[right=3.2cm, align=left] at (md) {Physical View\\(PTEs)};
...
This approach, pioneered by Sleator and Tarjan, is notably utilized by the BSD family of operating systems (FreeBSD, NetBSD) for their VM map structures. It trades the strict balance of other trees for an amortized efficiency that adapts to the specific runtime behavior of the process.

\subsubsection{Red-Black Trees}

In contrast, the Linux kernel employs \textbf{Red-Black Trees} (RB-Trees) for managing virtual memory areas (\texttt{vm\_area\_struct}). RB-Trees enforce a stricter balancing invariant, guaranteeing that the longest path from root to leaf is no more than twice as long as the shortest path. 
...
While simple, eager allocation is inefficient. Observation of real-world program behavior reveals that applications rarely utilize their entire allocated address space simultaneously. Instead, they exhibit \textbf{Locality of Reference}. As formalized by Peter J. Denning in the \textit{Working Set Model}, the subset of memory pages a process is actively using at any specific time interval (its working set) is typically a small fraction of its total virtual size.

Even in a system that does not utilize swap files (backing store on disk), understanding this theory is crucial for optimizing physical RAM usage and system responsiveness.
...
\begin{enumerate}
    ...
    \item \textbf{Resume:} The kernel executes the \texttt{IRET} instruction. The CPU retries the memory access, which now succeeds transparently.
\end{enumerate}
...
As long as the processes only read memory, they share the physical RAM. The moment either process attempts to modify a page, a write-protection fault occurs. The kernel handles this by allocating a new physical frame, copying the content of the specific page being modified, and remapping that single page as Read-Write for the faulting process. This ensures that the expensive operation of memory copying occurs only when strictly necessary, and only at the granularity of a single page.
